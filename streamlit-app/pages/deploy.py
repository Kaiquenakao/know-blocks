...
from utils.api import prepare_deploy, upload_to_s3, confirm_deploy
...
# ══════════════════════════════════════════════════════════════════════════════
if step == 1:
    st.markdown('<div class="kb-card-title">Upload Documents</div>', unsafe_allow_html=True)
    ...
    if uploaded:
        st.markdown('<div class="kb-card-title" style="margin-top:1.5rem;">Ready to configure</div>',
                    unsafe_allow_html=True)
        for f in uploaded:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_left:
        st.markdown('<div class="kb-card-title">Chunking Strategy</div>', unsafe_allow_html=True)
        ...
        if strategy == "Fixed Size":
            ...
            st.markdown(f'<div class="kb-info">▸ Groups {n_sent} sentences per chunk</div>',
                        unsafe_allow_html=True)

        elif strategy == "Paragraph-based":
            st.markdown('<div class="kb-info">▸ Splits on paragraph breaks — great for structured docs</div>',
                        unsafe_allow_html=True)

        elif strategy == "Semantic":
            thresh = st.slider("Similarity threshold", 0.50, 0.99, 0.85, step=0.01)
            st.markdown(f'<div class="kb-info">▸ Groups semantically similar sentences · threshold {thresh}</div>',
                        unsafe_allow_html=True)

        st.session_state._chunk_strategy = strategy

    with col_right:
        st.markdown('<div class="kb-card-title">Embedding Model</div>', unsafe_allow_html=True)
        st.markdown("""
        ...
        info = {
            "nomic-embed-text": ("768", "free · Ollama", "recommended", "Best quality/size ratio — ideal for most use cases"),
            "all-minilm":       ("384", "free · Ollama", "lightweight",  "Ultra fast — great for large document sets"),
        }
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Left: Metadata form ───────────────────────────────────────────────────
    with col_meta:
        st.markdown('<div class="kb-card-title">Document Metadata</div>', unsafe_allow_html=True)
        st.markdown("""
        <div class="kb-info">
        ...
        tags_html = "".join([f'<span class="kb-tag" style="margin-bottom:4px;">{t}</span>' for t in tags]) or "—"
        st.markdown(f"""
        <div class="kb-card">
    ...
    # ── Right: Chunk preview ──────────────────────────────────────────────────
    with col_preview:
        st.markdown('<div class="kb-card-title">Chunk Sampling</div>', unsafe_allow_html=True)
        st.markdown("""
    ...
    with col_next:
        if st.button("🚀 Deploy to Store"):
            ...
            if not f:
                st.error("Nenhum arquivo encontrado. Volte ao Step 1.")
            else:
                # ── Step 1: valida + gera presigned URL ───────────────────
                with st.spinner("Validando documento..."):
                    prepare = prepare_deploy(filename=f.name)

                if prepare.is_conflict:
                    st.markdown(f"""
                    <div class="kb-info" style="color:#c0392b; border-color:#c0392b33; background:#fff5f5;">
                        ❌ <strong>{f.name}</strong> já foi deployado anteriormente.
                    </div>
                    """, unsafe_allow_html=True)

                elif not prepare.success:
                    st.error(f"Erro ao preparar deploy: {prepare.message}")

                else:
                    # ── Step 2: upload direto pro S3 ──────────────────────
                    with st.spinner("Enviando arquivo para o S3..."):
                        uploaded = upload_to_s3(prepare.presigned_url, f)

                    if not uploaded:
                        st.error("Falha ao enviar o arquivo. Tente novamente.")

                    else:
                        # ── Step 3: confirma e inicia Step Function ────────
                        with st.spinner("Iniciando pipeline..."):
                            confirm = confirm_deploy(
                                upload_id      = prepare.upload_id,
                                s3_key         = prepare.s3_key,
                                filename       = f.name,
                                chunk_strategy = strategy,
                                chunk_size     = st.session_state._chunk_size,
                                overlap        = st.session_state._overlap,
                                embed_model    = model,
                                title          = doc_title,
                        ...
                        if not confirm.success:
                            st.error(f"Erro ao iniciar pipeline: {confirm.message}")
                        else:
                            st.session_state._execution_arn = confirm.execution_arn
                            st.session_state._deployed_at   = confirm.deployed_at
                            import zoneinfo
                            now = datetime.datetime.now(
                                tz=zoneinfo.ZoneInfo("America/Sao_Paulo")
                            ).strftime("%d/%m/%Y %H:%M:%S")
                            st.session_state.documents.append({
                                "name":          doc_title or f.name,
                                "filename":      f.name,
                                "description":   description,
                                "language":      language,
                                "doc_type":      doc_type,
                                ...
                                "embeddings":    0,
                                "deployed_at":   now,
                                "execution_arn": confirm.execution_arn,
                            })
                            st.session_state.total_chunks += len(sample_chunks)
